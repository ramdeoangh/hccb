...
class Hccb extends REST_Controller
{
  ...
  {
    $responseMessage = array('data' => array("user_id" => null, "course_id" => null), 'status' => false, 'message' => 'Unauthorize request');
  ...
  public function subscribe_course_post()
  {
    $responseMessage = array('data' => array("user_id" => null, "course_id" => null), 'status' => false, 'message' => 'Unauthorize request');
    $userdata = array();
    ...
    try {
      ...
      if (!isset($email) || empty($email)) {
        $responseMessage['message'] = 'invalid email id';
        $responseMessage['status'] = false;
        $this->set_response($responseMessage, REST_Controller::HTTP_UNAUTHORIZED);
        return;
      }
      if (!isset($inputData['student_name']) || empty($inputData['student_name'])) {
        $responseMessage['message'] = 'invalid student name';
        $responseMessage['status'] = false;
        $this->set_response($responseMessage, REST_Controller::HTTP_UNAUTHORIZED);
        return;
      }
      if (!isset($inputData['college_code']) || empty($inputData['college_code'])) {
        $responseMessage['message'] = 'invalid college code';
        $responseMessage['status'] = false;
        $this->set_response($responseMessage, REST_Controller::HTTP_UNAUTHORIZED);
        return;
      }
      if (!isset($inputData['college_name']) || empty($inputData['college_name'])) {
        $responseMessage['message'] = 'invalid college name';
        $responseMessage['status'] = false;
        $this->set_response($responseMessage, REST_Controller::HTTP_UNAUTHORIZED);
        return;
      }
      if (!isset($inputData['branch_name']) || empty($inputData['branch_name'])) {
        $responseMessage['message'] = 'invalid branch name';
        $responseMessage['status'] = false;
        $this->set_response($responseMessage, REST_Controller::HTTP_UNAUTHORIZED);
        return;
      }
      if (!isset($inputData['district']) || empty($inputData['district'])) {
        $responseMessage['message'] = 'invalid district';
        $responseMessage['status'] = false;
        $this->set_response($responseMessage, REST_Controller::HTTP_UNAUTHORIZED);
        return;
      }
      if (!isset($inputData['university']) || empty($inputData['university'])) {
        $responseMessage['message'] = 'invalid university';
        $responseMessage['status'] = false;
        $this->set_response($responseMessage, REST_Controller::HTTP_UNAUTHORIZED);
        return;
      }
      ...
      if ($validity === 'unverified_user' || $validity == true) {
        if ($validity === true) {
          ...
          if ($response['status'] == 403) {
            ...
            $responseMessage['status'] = false;
            return $this->set_response($responseMessage, REST_Controller::HTTP_FORBIDDEN);
          }
        }
      }
      ...
      if ($course_details['is_free_course'] == 1) {
        ...
        if ($this->db->get_where('enrol', ['course_id' => $course_id, 'user_id' => $logged_in_user_details['user_id']])->num_rows() > 0) {
          ...
          $this->db->where('course_id', $course_id);
          $this->db->update('enrol', $data);
          $responseMessage['message'] = 'already subscribed';
          $responseMessage['status'] = true;
          $responseMessage['data']["user_id"] = $logged_in_user_details['user_id'];
          $responseMessage['data']["course_id"] = $course_id;
        } else {
          $data['course_id'] = $course_id;
          $data['user_id']   = $logged_in_user_details['user_id'];
          $data['date_added'] = strtotime(date('D, d-M-Y'));          
          $encryptedValue=$this->user_model->encrypt_decrypt('encrypt', $logged_in_user_details['user_id'].'-'.$course_id.'-'.strtotime(date('D, d-M-Y')));
          $data['token']=$encryptedValue;
          $this->db->insert('enrol', $data);
        ...
        $responseMessage['message'] = 'This course is not free';
        $responseMessage['status'] = false;
      }
    } catch (Exception $th) {
      $responseMessage['message'] = 'something went wrong. contact for support.';
      $responseMessage['status'] = false;
      return $this->set_response($responseMessage, REST_Controller::HTTP_FORBIDDEN);
    }
  ...
  public function course_progress_post()
  {
    ...
    try {
      ...
      $res=$this->api_model->hccb_update_watch_history_with_duration_post($user_id, $course_id);   
      $responseMessage=$res;
    } catch (Exception $th) {     
  ...
  public function user_courses_get()
  {
    $responseMessage = array('data' => null, 'status' => 401, 'message' => 'Unauthorize request');

    if (isset($_GET['user_id']) && !empty($_GET['user_id'])) {
      $user_id = $_GET['user_id'];
      $response = $response = $this->api_model->my_courses_get($user_id);
      $responseMessage['message'] = 'success';
